...
import { scoreTypes } from "../events/count.ts";
...
export const createDisplay = (embed: EmbedBuilder, props: DisplayProps) => {
  const body = [
    [
      "Last Count",
      props.lastCount,
      props.lastCountMemberId,
      props.lastCountTimestamp,
    ],
    [
      "Highest Count",
      props.highestCount,
      props.highestCountMemberId,
      props.highestCountTimestamp,
    ],
  ] as const;
  ...
  const rows = [
    [
      scoreTypes.valid.label,
      formatScore(scoreValid, scoreValid - scoreHighest, breakdownTotal),
    ],
    [
      scoreTypes.highest.label,
      formatScore(scoreHighest, scoreHighest, breakdownTotal),
    ],
    [
      scoreTypes.graced.label,
      formatScore(scoreGraced, scoreGraced, breakdownTotal),
    ],
    [
      scoreTypes.invalid.label,
      formatScore(scoreInvalid, scoreInvalid, breakdownTotal),
    ],
  ];
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addUserOption((option) =>
          option.setName("member").setDescription("The member to view")
        )
    )
    .addSubcommand((sub) =>
      sub.setName("server").setDescription("View the stats of this server")
    )
    .addSubcommand((sub) =>
      sub
        .setName("user")
    ...
    if (interaction.options.getSubcommand() === "server") {
      ...
      if (!data) {
        embed.setDescription(
          user.bot
            ? "Computers can't do math, silly."
            : "This user hasn't counted yet.",
        );
        await interaction.reply({ embeds: [embed] });
        return;
      }
